...
- OpenAI 金鑰只讀 server `process.env.OPENAI_API_KEY`；無 `NEXT_PUBLIC_`、前端金鑰欄位、localStorage 金鑰、console key 輸出。
...
- 45 秒 timeout、maxRetries=0、5000 output tokens、`store:false`，錯誤訊息不回傳 provider trace 或 request/response 全文。
...
- 仍需在 OpenAI project 設定貨幣支出限制；應用配額限制的是請求數。
...
- `store:false` 不代表第三方零保留或合規認證；請閱讀 OpenAI 當前資料政策。
- 未做外部滲透測試、正式 iOS Safari 實機驗收、分散式負載驗收。

## 上線設定

使用有 HTTPS 的私人 Sites / Cloudflare OpenNext 主機與 D1 DB；設定真實 `APP_ORIGIN`、`OPENAI_API_KEY`、`OPENAI_MODEL`、強隨機 `AI_ACCESS_TOKEN`。先保持私人存取。UI 設定頁輸入 App 存取碼取得一小時 cookie。金鑰輪替需主機端操作。未配置保護時 production API fail closed，其他排盤功能仍可用。
