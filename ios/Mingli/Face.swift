...
func analyzeFaceImage(_ data: Data) -> Output {
guard data.count > 0, data.count <= 15_000_000,
    ...
    return Output(faces: [], error: "照片格式或尺寸不支援。請選擇 15 MB、3200 萬像素以下的清晰 JPG／PNG（至少 160 × 160）。")
}
do {
    ...
    if observations.count > 1 {
        return Output(faces: [], error: "照片中有多張臉，請改用只有一人的正面照片。")
    }
    ...
    return Output(faces: faces, error: nil, width: image.width, height: image.height, photo: "data:image/jpeg;base64," + (preview as Data).base64EncodedString())
} catch {
    return Output(faces: [], error: "本機臉部辨識未完成，請更換照片後重試。")
}
